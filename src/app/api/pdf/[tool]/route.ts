  ...
  try { 
    ...
    switch (tool) {
      ...
      case 'pdf-to-word':
        ...
        filename = path.basename(inputPaths[0], path.extname(inputPaths[0])) + ".docx";
        break;

      case 'pdf-to-txt':
        resultPath = await PDFService.convertPDFToTXT(inputPaths[0], uploadDir, 'txt');
        contentType = "text/plain";
        filename = path.basename(inputPaths[0], path.extname(inputPaths[0])) + ".txt";
        break;
      case 'pdf-to-excel':
        resultPath = await PDFService.convertWithLibreOffice(inputPaths[0], uploadDir, 'xlsx');
        contentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
        filename = path.basename(inputPaths[0], path.extname(inputPaths[0])) + ".xlsx";
        break;

      case 'pdf-to-powerpoint':
        resultPath = await PDFService.convertPDFToPPT(inputPaths[0], uploadDir, 'pptx');
        contentType = "application/vnd.openxmlformats-officedocument.presentationml.presentation";
        filename = path.basename(inputPaths[0], path.extname(inputPaths[0])) + ".pptx";
        break;
    ...
    case "pdf-to-tiff": {
      ...
      filename = path.basename(inputPaths[0], path.extname(inputPaths[0])) + ".zip";
  ...
  case 'pdf-to-zip':
    ...
    break;
      ...
      case 'pdf-to-jpg':
        ...
        filename = path.basename(inputPaths[0], path.extname(inputPaths[0])) + ".jpg";
        break;
